...
class PensionSchemeReturnSippGETRequestService extends HttpClient {
  val host: String   = TestConfiguration.url("pension-scheme-return-sipp")
  val psrURL: String = s"$host/psr"

  def getStandardPSR(authToken: String, assetClass: String): StandaloneWSRequest#Self#Response =
    Await.result(
      get(
